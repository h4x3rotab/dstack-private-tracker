...
def initialize_web3_contract(provider_url, contract_address, contract_abi, private_key):
    ...
    try:
        ...
        if not w3.is_connected():
            sys.stderr.write(
                f"Error: Not connected to Web3 provider at {provider_url}\n")
            return None, None, None

        contract_address_checksum = w3.to_checksum_address(contract_address)
        contract = w3.eth.contract(
            address=contract_address_checksum, abi=contract_abi)

        owner_account = w3.eth.account.from_key(private_key)
        sys.stdout.write(
            f"Connected to Web3. Owner account: {owner_account.address}\n")
...
def add_user_to_contract(username, initial_download_size, initial_upload_size):
    """
    Adds a new user to the smart contract using the addUser function.
    """
    global w3_instance, reputation_contract, owner_eth_account

    if not (w3_instance and reputation_contract and owner_eth_account):
        sys.stderr.write(
            "Smart contract interaction not initialized or failed. Skipping addUser.\n")
        return False  # Indicate failure

    try:
        # Using dummy salt and passwordHash.
        # In a real application, these should be securely generated/retrieved
        # and match your application's user management logic.
        # An empty string or a generic placeholder if salt is not critical for initial add
        dummy_salt = ""
        # bytes32(0) in Solidity is b'\x00' * 32 in Python
        dummy_password_hash = b'\x00' * 32

        sys.stdout.write(
            f"Attempting to add new user '{username}' to smart contract...\n")
        transaction = reputation_contract.functions.addUser(
            username,
            dummy_salt,
            dummy_password_hash,
            initial_download_size,  # Typically 0 for a new user
            initial_upload_size    # Typically 0 for a new user
        ).build_transaction({
        ...
        tx_receipt = w3_instance.eth.wait_for_transaction_receipt(
            tx_hash, timeout=120)
        sys.stdout.write(
            f"Transaction receipt for addUser '{username}': Status={tx_receipt.status}, Block={tx_receipt.blockNumber}\n")

        if tx_receipt.status == 1:
            sys.stdout.write(
                f"Successfully added user '{username}' to smart contract.\n")
            return True
        else:
            sys.stderr.write(
                f"Transaction for addUser '{username}' failed on smart contract (status 0).\n")
            return False

    except ValueError as ve:
        sys.stderr.write(
            f"Error building transaction for addUser '{username}': {ve}\n")
        return False
    except TransactionNotFound as e:
        sys.stderr.write(
            f"Transaction for addUser '{username}' not found on chain (possibly dropped or timed out): {e}\n")
        return False
    except Exception as e:
        sys.stderr.write(
            f"Unhandled error interacting with smart contract for addUser '{username}': {e}\n")
        return False
...
def update_smart_contract_reputation(username, new_download_size, new_upload_size):
    ...
    If the user does not exist on the smart contract, it will attempt to add them first.
    """
    global w3_instance, reputation_contract, owner_eth_account

    if not (w3_instance and reputation_contract and owner_eth_account):
        sys.stderr.write(
            "Smart contract interaction not initialized or failed. Skipping update.\n")
        return

    user_exists_on_chain = False
    on_chain_download_size = 0
    on_chain_upload_size = 0

    try:
        current_user_data = reputation_contract.functions.getUserData(
            username).call()
        ...
        # A passwordHash of bytes32(0) usually indicates the user does not exist in the mapping.
        user_exists_on_chain = on_chain_password_hash != b'\x00' * 32

        sys.stdout.write(
            f"User '{username}' on-chain stats: Download={on_chain_download_size}, Upload={on_chain_upload_size}\n")

    except Exception as e:
        # If getUserData call fails (e.g., due to network error or contract revert for non-existent user),
        # we treat the user as not existing on chain for this logic.
        sys.stderr.write(
            f"Error getting current user data for '{username}' from contract: {e}. Assuming user not found or call failed.\n")
        user_exists_on_chain = False
    ...
    if not user_exists_on_chain:
        sys.stdout.write(
            f"User '{username}' does not exist on smart contract. Attempting to add.\n")
        # For a new user, initial download/upload sizes will be 0.
        # The subsequent update logic will then handle the actual new_download_size/new_upload_size.
        add_success = add_user_to_contract(username, 0, 0)
        if not add_success:
            sys.stderr.write(
                f"Failed to add user '{username}' to smart contract. Cannot proceed with update.\n")
            return
        # After successful addition, the user now exists on-chain, and we can proceed to update them.
        # We reset on-chain sizes to 0 for the comparison, as the user was just added with 0s.
        on_chain_download_size = 0
        on_chain_upload_size = 0
        user_exists_on_chain = True  # Now the user effectively exists for subsequent logic

    # 2. Compare values (only if user now exists)
    update_needed = False
    # Only update if the new reported size is GREATER than the current on-chain size.
    # This prevents unnecessary transactions and potential issues if a client sends old data.
    if new_download_size > on_chain_download_size:
        sys.stdout.write(
            f"New download size {new_download_size} > on-chain download size {on_chain_download_size}. Update needed.\n")
        update_needed = True
    if new_upload_size > on_chain_upload_size:
        sys.stdout.write(
            f"New upload size {new_upload_size} > on-chain upload size {on_chain_upload_size}. Update needed.\n")
        update_needed = True

    if not update_needed:
        sys.stdout.write(
            f"No increase in download/upload for user '{username}'. Skipping smart contract update.\n")
        return

    # 3. Proceed with update if needed
    sys.stdout.write(
        f"Initiating update for user '{username}' on smart contract...\n")
    try:
        ...
        tx_receipt = w3_instance.eth.wait_for_transaction_receipt(
            tx_hash, timeout=120)
        sys.stdout.write(
            f"Transaction receipt for '{username}': Status={tx_receipt.status}, Block={tx_receipt.blockNumber}\n")

        if tx_receipt.status == 1:
            sys.stdout.write(
                f"Successfully updated user '{username}' on smart contract.\n")
        else:
            sys.stderr.write(
                f"Transaction for user '{username}' failed on smart contract (status 0).\n")

    except ValueError as ve:
        sys.stderr.write(
            f"Error building transaction for user '{username}': {ve}\n")
    except TransactionNotFound as e:
        sys.stderr.write(
            f"Transaction for user '{username}' not found on chain (possibly dropped or timed out): {e}\n")
    except Exception as e:
        sys.stderr.write(
            f"Unhandled error interacting with smart contract for user '{username}': {e}\n")
...
def query_sqlite_db(db_path, query):
    ...
    try:
        ...
        sys.stderr.write(
            f"An unexpected error occurred during key loading: {e}\n")
        return []
    finally:
        if conn:
            conn.close()
...
class RequestLoggerHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if original_uri:
            ...
            if len(original_path_segments) >= 3 and original_path_segments[1] == 'announce':
                ...
                self.process_and_log_announce_params(
    ...
    def process_and_log_announce_params(self, params, auth_key):
        ...
        Also calls smart contract to add or update user reputation.
        """
        needs_update = False
        ...
        uploaded_val = int(
            uploaded_val_str) if uploaded_val_str is not None else 0
        downloaded_val = int(
            downloaded_val_str) if downloaded_val_str is not None else 0
        ...
        if auth_key is not None:
            # Get username from the pre-loaded keys map
            username = self.keys.get(auth_key)
            if username is not None:
                # Initialize user_stats entry if it doesn't exist
                if auth_key not in self.user_stats:
                    self.user_stats[auth_key] = {
                ...
                if info_hash_val:
                    ...
                    if auth_key not in self.torrents[info_hash_val]:
                        self.torrents[info_hash_val][auth_key] = {
                            'uploaded': 0, 'downloaded': 0}
                ...
                if needs_update:
                    ...
                    self.log_message(
                        f"  User {username} overall stats: Uploaded: {self.user_stats[auth_key]['uploaded']}, Downloaded: {self.user_stats[auth_key]['downloaded']}")
                    self.log_message("-----------------------")

                    update_smart_contract_reputation(
                        username,
                        # downloadSize for contract
                        self.user_stats[auth_key]['downloaded'],
    ...
    def do_PUT(self):
        ...
        content_length = int(self.headers.get('Content-Length', 0))
        put_body = self.rfile.read(content_length).decode(
            'utf-8', errors='ignore')
        ...
        self.end_headers()
        self.wfile.write(
            "<html><body><h1>PUT Request Received!</h1></body></html>".encode('utf-8'))
    ...
    def do_DELETE(self):
        ...
        self.end_headers()
        self.wfile.write(
            "<html><body><h1>DELETE Request Received!</h1></body></html>".encode('utf-8'))
...
def run_server():
    ...
    keys_map = {u['tracker_key']: u['username']
                for u in db_results} if db_results else {}
    ...
    shared_server_data = {
        ...
        'user_stats': {k: {'uploaded': 0, 'downloaded': 0} for k in keys_map.keys()}
    }
